...
<h3></h3>
<h3>MODIFICA VALORE BENE</h3>

<form action="" method="POST" name="valorimag">

<?php
$id = $_GET['id'];
$q = $x->query('SELECT * FROM etichette WHERE id='.$id.'');
$d = $q->fetch_array();
?>

<input type="text" name="nome" id="nome" value="<?php echo $d['campo'];?>" required>
<input type="submit" name="bene" value="MODIFICA">
...
if(isset($_POST['bene'])){
	$nome = $_POST['nome'];
	$q = $x->query('UPDATE etichette SET campo="'.$nome.'" WHERE id="'.$id.'"');
