...
$q = $x->query('SELECT * FROM magazzini WHERE id='.$id.'');
$d = $q->fetch_array();
?>

<input type="text" name="magaz" id="magaz" value="<?php echo $d['note'];?>" size="45px">
...
if(isset($_POST['etmag'])){
	$magaz = $_POST['magaz'];
	$q = $x->query('UPDATE magazzini SET note="'.$magaz.'" WHERE id="'.$id.'"');
	header("Location: magazzino.php?id=$magazzino");
}